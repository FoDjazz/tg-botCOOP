...
async def confirm_and_publish(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # Сценарий 10: финальная проверка что время не в прошлом
    hour = data.get('hour', DEFAULT_HOUR)
    minute = data.get('minute', DEFAULT_MINUTE)
    announce_date_check = data.get('announce_date', tz_now().strftime("%Y-%m-%d"))
    try:
        chosen_dt = datetime.strptime(
            f"{announce_date_check} {hour:02d}:{minute:02d}", "%Y-%m-%d %H:%M"
        )
        if chosen_dt < tz_now():
            await callback.answer(
                "❌ Время анонса уже прошло! Вернись назад и выбери другое время.",
                show_alert=True
            )
            return
...
# Кнопки меню — прерывают FSM создания анонса
_ANNOUNCE_MENU_BUTTONS = {
...
async def game_text_invalid(message: Message, state: FSMContext):
    if not message.text or message.text.startswith("/"): return
    if message.text in _ANNOUNCE_MENU_BUTTONS:
        await state.clear(); return
    await message.answer("⚠️ Используй кнопки выше для выбора игры.")


@router.message(AnnounceForm.choosing_date)
async def date_text_invalid(message: Message, state: FSMContext):
    if not message.text or message.text.startswith("/"): return
    if message.text in _ANNOUNCE_MENU_BUTTONS:
        await state.clear(); return
...
async def time_text_invalid(message: Message, state: FSMContext):
    if not message.text or message.text.startswith("/"): return
    if message.text in _ANNOUNCE_MENU_BUTTONS:
        await state.clear(); return
...
async def participants_text_invalid(message: Message, state: FSMContext):
    if not message.text or message.text.startswith("/"): return
    if message.text in _ANNOUNCE_MENU_BUTTONS:
        await state.clear(); return
...
# === Сценарий 8: восстановление после потери FSM при перезагрузке ===
# MemoryStorage не персистентен — после рестарта бота состояния теряются.
# Если пользователь был в середине создания анонса и бот перезагрузился,
# следующее сообщение попадает сюда без активного состояния.
# Решение: при /start и /announce всегда делаем state.clear() перед началом.
# Дополнительно — если пришло фото без активного состояния, предлагаем начать заново.

@router.message(F.photo)
async def photo_without_state(message: Message, state: FSMContext):
    """Сценарий 8: фото пришло вне контекста создания анонса."""
    current = await state.get_state()
    # Если уже в процессе создания — не перехватываем (роутер разберётся)
    if current is not None:
        return
    # Фото без контекста — предлагаем начать создание анонса
    if message.from_user.id == ADMIN_ID:
        from config import ADMIN_ID as _ADMIN_ID
        await state.set_state(AnnounceForm.waiting_photo)
        # Обрабатываем фото как шаг 1
        photo_file_id = message.photo[-1].file_id
        await state.update_data(photo_file_id=photo_file_id)
        games = await db.get_all_games()
        await state.set_state(AnnounceForm.choosing_game)
...
async def cmd_backup(message: Message, bot: Bot):
    ...
    if message.from_user.id != ADMIN_ID:
        return
    from handlers.notifications import send_backup
    await message.answer("\u23f3 Создаю бэкап...")
    await send_backup(bot)
    await message.answer("\u2705 Готово — файл базы отправлен выше.")
