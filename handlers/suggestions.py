...
# Кнопки меню которые должны прерывать FSM предложения
_MENU_BUTTONS_SUGGEST = {
    "📋 Меню", "⭐ Оценить игру", "📖 Обзоры", "🎮 Анонсы",
    "🚫 Сегодня не играю", "⬅️ Назад в меню",
    "👤 Я как игрок", "👤 Меню пользователя",
    "📢 Анонс", "➕ Создать анонс", "👥 Участники",
    "🎮 Игры", "⚙️ Настройки",
}
...
async def process_steam_link(message: Message, state: FSMContext, bot: Bot):
    ...
    # Кнопка меню — сбрасываем FSM и передаём управление дальше
    if text in _MENU_BUTTONS_SUGGEST:
        await state.clear()
        return
