...
from config import GROUP_CHAT_ID, ANNOUNCE_TOPIC_ID, ADMIN_ID
...
async def send_backup(bot: Bot):
    ...
    if not os.path.exists(DB_PATH):
        logger.warning("Файл базы не найден, бэкап пропущен")
        return

    # Создаём папку если нет
    os.makedirs(BACKUP_DIR, exist_ok=True)

    # Делаем локальную копию с датой
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    backup_path = os.path.join(BACKUP_DIR, f"game_bot_{timestamp}.db")
    ...
    try:
        await bot.send_document(
            ...
            caption=f"🗄 Автобэкап базы данных\n📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}"
        )
...
async def schedule_auto_backup(bot: Bot):
    """
    Запускает автоматический бэкап каждые BACKUP_INTERVAL_DAYS дней.
    Отправляет файл базы администратору в Telegram.
    """
    while True:
        await asyncio.sleep(BACKUP_INTERVAL_DAYS * 24 * 3600)
        logger.info("🗄 Запуск автобэкапа...")
        try:
...
# Хранилище задач таймаута переноса: (announcement_id, user_id) -> asyncio.Task
# Если пользователь не выбрал дату — через N часов уведомляем админа (сценарий 1)
reschedule_timeout_tasks: dict[tuple, asyncio.Task] = {}
RESCHEDULE_TIMEOUT_HOURS = 3
...
async def schedule_vote_reminders(bot: Bot, announcement_id: int,
    ...
    elif delay_10 <= 0 and (game_dt - current).total_seconds() > 0:
        # До игры меньше 10 минут — запускаем авто-отмену немедленно (сценарий 12)
        tasks.append(asyncio.create_task(
            _auto_cancel_if_not_voted(bot, announcement_id, 0)
        ))
        logger.info(f"  → Авто-отмена НЕМЕДЛЕННО (до игры < 10 мин)")
...
# === ТАЙМАУТ ПЕРЕНОСА (сценарий 1) ===

def schedule_reschedule_timeout(bot, announcement_id: int, user_id: int):
    """
    Запускает таймер: если пользователь не выбрал дату через RESCHEDULE_TIMEOUT_HOURS —
    бот уведомляет админа.
    Вызывается в voting.py при нажатии ❌.
    """
    key = (announcement_id, user_id)
    # Отменяем предыдущий таймер если был
    if key in reschedule_timeout_tasks:
        reschedule_timeout_tasks[key].cancel()

    task = asyncio.create_task(
        _reschedule_timeout_handler(bot, announcement_id, user_id)
    )
    reschedule_timeout_tasks[key] = task
    logger.info(
        f"Таймаут переноса запущен: анонс #{announcement_id}, "
        f"user_id={user_id}, через {RESCHEDULE_TIMEOUT_HOURS}ч"
    )


def cancel_reschedule_timeout(announcement_id: int, user_id: int):
    """
    Отменяет таймаут переноса.
    Вызывается когда пользователь всё-таки выбрал дату.
    """
    key = (announcement_id, user_id)
    task = reschedule_timeout_tasks.pop(key, None)
    if task:
        task.cancel()
        logger.info(f"Таймаут переноса отменён: анонс #{announcement_id}, user_id={user_id}")


async def _reschedule_timeout_handler(bot, announcement_id: int, user_id: int):
    """Срабатывает если пользователь так и не выбрал дату переноса."""
    await asyncio.sleep(RESCHEDULE_TIMEOUT_HOURS * 3600)

    key = (announcement_id, user_id)
    reschedule_timeout_tasks.pop(key, None)

    user = await db.get_user(user_id)
    user_name = (
        f"@{user['username']}" if user and user['username']
        else (user['display_name'] if user else str(user_id))
    )

    logger.info(f"Таймаут переноса сработал: анонс #{announcement_id}, {user_name}")

    # Пишем пользователю что время вышло
    try:
        await bot.send_message(
            chat_id=user_id,
            text=(
                "⏰ Прошло 3 часа — ты так и не выбрал дату.\n"
                "Админ сам решит что делать с игрой."
            )
        )
    except Exception:
        pass
    ...
    try:
        await bot.send_message(
            chat_id=ADMIN_ID,
            text=(
                f"⏰ {user_name} не выбрал дату переноса за {RESCHEDULE_TIMEOUT_HOURS}ч\n"
                f"Анонс #{announcement_id}\n\n"
                f"Выбери дату сам или удали анонс."
            ),
            reply_markup=admin_kb
        )
    except Exception as e:
        logger.error(f"Не удалось уведомить админа о таймауте переноса: {e}")
