...
from config import (
    GROUP_CHAT_ID, ANNOUNCE_TOPIC_ID,
...
async def save_and_schedule(callback: CallbackQuery, bot: Bot,
    ...
    """
    Сохраняет перенос, создаёт новый анонс в БД (message_id=NULL — ещё не опубликован),
    и планирует его публикацию за HOURS_BEFORE_ANNOUNCE часов до начала.
    """
...
async def publish_announcement(bot: Bot, announcement_id: int):
    ...
    text = (
        f"{date_display} {announcement['start_time']} – {announcement['end_time']}! {game['emoji']}\n\n"
        f"Участники:\n{' '.join(mentions)}"
    )
